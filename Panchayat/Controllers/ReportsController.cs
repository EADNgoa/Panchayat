        ...
        public ActionResult RVReport(int RptYear, int SubLedgerID)
        {
            if (ModelState.IsValid )
            {
                ...
                IEnumerable<RVReport> res = (sl.Ledger1.IsIncome) ?
                    ...
                    : db.Vouchers.Where(f => f.SubLedgerID == SubLedgerID && ((DateTime)f.PayDate).Year == RptYear).Select(f => new RVReport { Amount = (decimal)f.Amount, RVid = f.VoucherID, Tdate = (DateTime)f.PayDate,form6= (bool)f.Form6 }).ToList();
